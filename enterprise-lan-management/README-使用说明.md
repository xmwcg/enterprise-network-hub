...
└── scripts/
    ...
    ├── lib-diskclean.ps1    # ★C盘瘦身核心库：白名单+安全扫描+备份+删除+系统健康检查
    ├── diskclean.ps1        # ★C盘瘦身入口：CLI 向导 + 简易图形（WinForms）
...
## C盘瘦身（系统垃圾清理，M3.5 · 解决"无论多大C盘都会满"痛点）

面向广大用户/中小企业"**C盘空间总是用一段时间就满**"的普遍痛点：本工具**只删除系统产生的垃圾**（更新缓存、临时文件、回收站、浏览器缓存、错误报告、缩略图缓存、组件存储冗余等），**绝不触碰个人文档、企业文件或任何应用业务数据**。

**安全铁律（对应"100% 安全"边界，诚实表述而非承诺绝对零风险）**：
- **仅白名单**：可删项全部来自 `lib-diskclean.ps1` 硬编码白名单（按"类别→路径→风险等级"），绝不"全盘找大文件就删"；`AppData` 仅精确匹配缓存子目录。
- **用户可选删除/跳过**：扫描结果逐项中文列出，可勾选/跳过；图形界面小白友好。
- **中文明文 + 风险告知**：每一项都有中文名称、路径、预估大小、风险等级与说明。
- **一键备份到 D: 或 U 盘**：删除前先把待删项复制到指定备份盘，并生成 `restore.ps1` + `恢复说明.txt`。
- **防误删 & 可恢复**：备份后双击 `restore.ps1` 即可原样还原；删除后做系统健康检查，异常自动从备份还原。
- **确认后才删除**：扫描 → 人工确认 → 才出现"执行删除"；任何高风险项（如休眠文件 `hiberfil.sys`）默认不出现，仅在高级选项提供并强提示。

```powershell
# 图形界面（推荐小白；无参数即图形，逐项勾选）
.\diskclean.ps1
# 命令行向导（交互式）
.\diskclean.ps1 -Cli
# 仅扫描、不删除（预览本机可清理量）
.\diskclean.ps1 -ScanOnly
# 含高级高风险项（如休眠文件）
.\diskclean.ps1 -Advanced
```

> 备份盘选择：运行时会列出 D: 等本地盘与 U 盘（可移动磁盘优先），亦可选择"不备份直接删除"（不推荐）。
